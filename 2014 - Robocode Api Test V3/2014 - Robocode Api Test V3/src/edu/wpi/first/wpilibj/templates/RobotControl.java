...
public class RobotControl {
   ...
   public void setTurnLeft(int degrees) {
    ...
    public double inchToTick(int inch) {
        return (10.24 * ((100 * inch) - .5)) / (Math.PI * 6);
    }

    public void setForward(double speed, int inchDistance, boolean brake) {
        resetEncoders();
        double ticksToDest = inchToTick(inchDistance);
        double initGyro = driveGyro.getAngle();
        double proportionalSpeed = speed;
        double encoderInit = (Math.abs(leftEnc.getRaw()) + Math.abs(rightEnc.getRaw())) / 2;
        System.out.println("encoder init val: " + encoderInit);
        ...
        if (brake) {
            ...
            while (getEncoders() - encoderInit <= ticksToDest) { //While we are not at our destination
                //Move forward

                setLeft(proportionalSpeed);
                setRight(proportionalSpeed);

                if (Math.abs(initGyro - driveGyro.getAngle()) > 5) { //If we are off path
                    //correct the path

                    //System.out.println("Off path by " + Math.abs(initGyro - driveGyro.getAngle()) + ".");
                }
            }
